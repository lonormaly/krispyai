...
- **`[!HANDOFF]` with no form** → the widget shows a minimal name + contact capture; on
  submit it `POST`s to `/api/contact` (a back-compat shim that maps onto the same lead
  fan-out).
